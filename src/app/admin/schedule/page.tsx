    ...
    <>
      ...
      <main className="flex-1 w-full mx-auto p-6 md:p-8 overflow-hidden flex flex-col h-[calc(100vh-120px)]">
        <div className="mb-6 flex justify-between items-end shrink-0">
          <div>
            <h2 className="text-2xl font-bold text-white">Agenda da Semana</h2>
            <p className="text-slate-400 mt-1">Acompanhe os agendamentos futuros.</p>
          </div>
          <div className="flex gap-2">
            <button className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-sm font-medium hover:bg-slate-800/20">Hoje</button>
            <div className="flex bg-slate-800/50 p-1 rounded-lg">
              <button className="px-3 py-1 bg-slate-900 shadow-sm rounded-md text-sm font-medium">Semana</button>
              <button className="px-3 py-1 text-slate-400 text-sm font-medium hover:text-slate-700 disabled:opacity-50" disabled>Mês</button>
            </div>
            <NewBookingButton />
          </div>
        </div>
        ...
        <div className="flex-1 bg-slate-900 rounded-xl shadow-sm border border-slate-800 flex flex-col overflow-hidden">
          {/* Header row (Days) */}
          <div className="flex border-b border-slate-800 bg-slate-50/50">
            <div className="w-20 shrink-0 border-r border-slate-800"></div>
            <div className="flex-1 grid grid-cols-7">
              {weekDays.map((date, i) => (
                <div key={i} className="py-3 flex flex-col items-center justify-center border-r border-slate-800 last:border-r-0">
          ...
          <div className="flex-1 overflow-y-auto flex custom-scrollbar relative">
            <div className="w-20 shrink-0 border-r border-slate-800 relative">
            ...
            <div className="flex-1 grid grid-cols-7 relative">
              ...
              <div className="absolute inset-0 grid grid-cols-7 pointer-events-none">
                {Array.from({ length: 7 }).map((_, i) => (
                  <div key={i} className="border-r border-slate-800 last:border-r-0 h-full relative">
                    {timeSlots.map((_, j) => (
                      <div key={j} className="h-24 border-b border-slate-800/50"></div>
                    ))}
                  </div>
                ))}
              </div>
                    ...
                    <div key={colIndex} className="relative h-full">
                      {dayBookings.map(b => {
                        ...
                        return (
                          <div 
                            key={b.id} 
                            className="absolute left-1 right-1 rounded-md bg-teal-500/20 border-l-4 border-indigo-600 p-2 shadow-sm overflow-hidden group hover:z-10 transition-all hover:shadow-md cursor-pointer hover:h-auto hover:min-h-[96px]"
