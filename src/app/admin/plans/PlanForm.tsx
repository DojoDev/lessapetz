...
export default function PlanForm({ plan, isEditing = false }: PlanFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
      ...
      {/* Name */}
      <div>
        <label className="block text-sm font-medium text-slate-300 mb-1.5">Nome do Plano *</label>
        <input
          type="text"
          required
          value={form.name}
      ...
      {/* Monthly Price */}
      <div>
        <label className="block text-sm font-medium text-slate-300 mb-1.5">Preço Mensal (R$) *</label>
        <input
          type="number"
          inputMode="decimal"
          required
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Quota */}
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Quota de Serviços por Ciclo *</label>
          <input
            type="number"
            required
            min="0"
            ...
            placeholder="Ex: 4"
          />
          <p className="mt-1 text-xs text-slate-400">Nº de vezes que o serviço pode ser usado no ciclo.</p>
        </div>

        {/* Cycle Length */}
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Duração do Ciclo (dias) *</label>
          <input
            type="number"
            required
            min="1"
            ...
            placeholder="Ex: 30"
          />
          <p className="mt-1 text-xs text-slate-400">Normalmente 30 dias para mensal.</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="border border-slate-700 rounded-lg max-h-60 overflow-y-auto divide-y divide-slate-800/30">
          {services.length === 0 ? (
            <p className="text-sm text-slate-400 py-4 text-center">Carregando serviços...</p>
          ) : (
            services.map(s => (
              <label
                key={s.id}
                className={`flex items-center gap-3 px-4 py-3 min-h-[44px] cursor-pointer hover:bg-slate-800/20 transition-colors ${
                  form.includedServiceIds.includes(s.id) ? 'bg-teal-500/10' : ''
                }`}
              >
                <input
                  type="checkbox"
                  ...
                  className="w-5 h-5 text-teal-400 rounded border-slate-700 focus:ring-teal-500 shrink-0"
                />
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-slate-200">{s.name}</span>
                  {s.categoryName && (
                    <span className="ml-2 text-xs text-slate-400">({s.categoryName})</span>
                  )}
                </div>
              </label>
            ))
