      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 md:p-8">
        <div className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-400 mt-1">Gerencie os clientes e seus pets.</p>
          </div>
          <Link href="/admin/customers/new" className="px-4 py-2 bg-teal-500 hover:bg-teal-400 text-white font-medium rounded-lg shadow-sm transition-colors">
            Novo Cliente
          </Link>
        </div>
                  ...
                  customers.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-800/20 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        {c.cpf && <div className="text-xs text-slate-400 mt-1">CPF: {c.cpf}</div>}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/customers/${c.id}`} className="inline-flex items-center justify-center text-teal-400 hover:text-teal-300 bg-teal-500/10 hover:bg-teal-500/20 px-4 py-2 rounded-lg text-sm font-medium transition-colors min-h-[44px]">
                          Detalhes
                        </Link>
                      </td>
                    </tr>
                ...
                <div key={c.id} className="p-4 flex flex-col gap-3">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-white text-base leading-tight truncate">{c.fullName}</h3>
                      {c.cpf && <p className="text-xs text-slate-400 mt-1">CPF: {c.cpf}</p>}
                    </div>
                  </div>
                  
                  <div className="bg-slate-900/50 rounded-lg p-3 flex flex-col gap-1.5">
                  ...
                  <Link href={`/admin/customers/${c.id}`} className="mt-1 w-full flex items-center justify-center text-teal-400 bg-teal-500/10 hover:bg-teal-500/20 rounded-lg min-h-[44px] text-sm font-medium transition-colors">
                    Ver Detalhes
                  </Link>
                </div>
              ))
