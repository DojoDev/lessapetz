...
export default function AlertasPage() {
  ...
  return (
    <>
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white">Planos Próximos do Vencimento</h1>
        ...
        {loading ? (
          ...
          <div className="bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-12 text-center text-slate-400">
            Nenhum alerta no momento. Todos os planos ativos estão em dia ou com a cota consumida.
          </div>
        ) : (
          <div className="bg-slate-900 rounded-xl shadow-sm border border-slate-800 overflow-hidden divide-y divide-slate-800/50">
            {alerts.map(alert => (
              <div key={alert.id} className="p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <h3 className="font-semibold text-white">{alert.client_name}</h3>
                  <div className="text-sm text-slate-300 mt-1">
                    <span className="font-medium text-slate-200">{alert.plan_name}</span> • 
                  ...
                  <div className="text-sm text-slate-400 mt-2">
                    Uso: <span className="font-medium text-slate-300">{alert.uses_consumed} de {alert.total_quota}</span> consumidos 
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 bg-teal-500 hover:bg-teal-400 text-white text-sm font-medium rounded-lg disabled:opacity-50 transition-colors"
                >
