...
/** Default category images when no custom image is set */
const CATEGORY_DEFAULT_IMAGES: Record<string, string> = {
  'Banhos': '/assets/pet_bath_spa.png',
  'Banho': '/assets/pet_bath_spa.png',
  'Tosas': '/assets/pet_tosa_grooming.png',
  'Tosa': '/assets/pet_tosa_grooming.png',
  'Pacotes': '/assets/pet_package_spa.png',
};
const FALLBACK_IMAGE = '/assets/spa_dog.png';

function getImageUrl(item: Service | Course): string | null {
  if ('imageUrl' in item && item.imageUrl) return item.imageUrl;
  const categoryImage = CATEGORY_DEFAULT_IMAGES[item.category];
...
export default function CardItem({ item, type, whatsAppUrl }: CardItemProps) {
  ...
  return (
    <div className="group relative flex flex-col justify-between rounded-3xl bg-white border border-brand-gold/15 hover:border-brand-gold/45 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1.5 overflow-hidden">
      {/* Card Image */}
      {imageUrl && (
        <div className="relative w-full aspect-[16/9] overflow-hidden bg-brand-cream-dark">
          <img
            src={imageUrl}
            alt={item.title}
          ...
          <div className="absolute top-3 left-3">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider bg-white/90 backdrop-blur-sm text-brand-burgundy border border-brand-burgundy/10 shadow-sm">
              {item.category}
            </span>
          </div>
          ...
          {isCourse && (
            <div className="absolute top-3 right-3">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider backdrop-blur-sm shadow-sm ${
      ...
      <div className="flex-1 flex flex-col p-6 sm:p-8">
        <div className="flex-1">
          {/* Category tag (only shown when there's no image) */}
          {!imageUrl && (
          ...
          <div className="flex flex-wrap items-center gap-y-3 gap-x-4 mb-6 border-t border-dashed border-stone-200 pt-4">
            ...
            {/* Pet Size (services only) */}
            {'petSizeApplicability' in item && item.petSizeApplicability && item.petSizeApplicability !== 'all' && (
              <div className="flex items-center gap-1.5 text-stone-500 text-xs sm:text-sm">
                <svg className="w-4 h-4 text-brand-gold stroke-current stroke-2 fill-none" viewBox="0 0 24 24">
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z" />
                </svg>
                <span>Porte: <strong>{item.petSizeApplicability === 'small' ? 'Pequeno' : item.petSizeApplicability === 'medium' ? 'Médio' : 'Grande'}</strong></span>
              </div>
            )}
